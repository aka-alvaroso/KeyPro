...
const Test = ({
  ...
  const fetchText = async () => {
    const response = await axios.get(
      `${import.meta.env.VITE_API_URL}/text/get`,
      {
        params: {
          type: settings.type,
  ...
  useEffect(() => {
    const handleKeyPress = (event) => {
      ...
      if (
        typedChar === "Escape" ||
        (typedChar === "F5" && testResults.isReady) ||
        (typedChar === "r" && event.ctrlKey && testResults.isReady)
      ) {
        setTimedText([]);
        setTimedResults([]);
        setCursor(0);
      ...
      if (settings.mode === "timed" && timeRemaining <= 0) {
        ...
        if (sessionStorage.getItem("loggedIn") === "true" && !hasResultsSent) {
          axios
            .post(`${import.meta.env.VITE_API_URL}/user/update`, {
          ...
          axios
            .post(`${import.meta.env.VITE_API_URL}/test/save`, {
              id: generateTestId(),
              text: timedText.join(""),
