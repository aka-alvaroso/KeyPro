...
const History = ({
    ...
    const fetchHistory = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_URL}/test/user/${username}`
        );
  ...
  return (
    ...
    >
      ...
      <h1 className={`text-xl text-${theme}-text h-16`}>
        Historial de tests de
        <Link
          to={`/profile/${username}`}
      ...
      {history.length > 0 ? (
        ...
        >
          ...
          <div
            className={`w-full text-md text-center flex flex-col text-${theme}-text`}
          >
            {history.map((test, index) => (
              <Link
                to={`/test/${test.id}`}
                key={index}
                className={`w-full flex items-center py-2 hover:bg-${theme}-primary hover:bg-opacity-10 rounded-lg gap-4`}
              >
                <p className="w-1/6">
        ...
        <h5 className={`h-screen text-lg text-${theme}-text`}>
          No hay registros
        </h5>
      )}
